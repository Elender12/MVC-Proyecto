...
class Currante extends Model{
    ...
    public function chan($old,$new){

        $stmt = $this->conex->prepare("UPDATE LoginUsuarios SET Clave=? WHERE Id LIKE ? AND Clave LIKE ?");
        $stmt->bind_param("sss", $n,$id,$o );
        $id=$_SESSION["uid"];
        $n=$new;
        $o=$old;
        $stmt->execute();
    ...
    public function anular($dia){

        $stmt = $this->conex->prepare("DELETE FROM Imputaciones WHERE Id LIKE ? AND Dia LIKE ?");
        $stmt->bind_param("ss", $id,$dia );
        $id=$_SESSION["uid"];
        $d=$dia;
        $stmt->execute();
